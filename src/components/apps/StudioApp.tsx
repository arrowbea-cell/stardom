...
import { Music, Mic2, Home, Lock, Camera, Star, Zap, TrendingUp, DollarSign, Check, Archive, Disc3, Headphones, Speaker, MonitorSpeaker, Radio, Megaphone, ListMusic, BarChart3, Smartphone, Building2, CircleDot } from 'lucide-react';
...
type Section = 'main' | 'record' | 'vault' | 'home-studio' | 'analytics';

const STUDIO_ICONS: Record<string, any> = {
  '🎵': Music, '🏠': Home, '🎙️': Mic2, '🎤': Mic2, '💎': Disc3,
  '🎧': Headphones, '🔊': Speaker, '🎹': MonitorSpeaker,
};

const PROMOTION_OPTIONS = [
  { id: 'none', name: 'No Promotion', cost: 0, multiplier: 1.0, icon: Music, desc: 'Organic growth only' },
  { id: 'social', name: 'Social Media Ads', cost: 1000, multiplier: 1.5, icon: Smartphone, desc: '+50% stream boost' },
  { id: 'playlist', name: 'Playlist Placement', cost: 3000, multiplier: 2.0, icon: ListMusic, desc: '2x stream boost' },
  { id: 'radio', name: 'Radio Campaign', cost: 8000, multiplier: 3.0, icon: Radio, desc: '3x stream boost' },
  { id: 'billboard', name: 'Billboard + Press', cost: 20000, multiplier: 5.0, icon: Building2, desc: '5x stream boost' },
];

const HOME_STUDIO_UPGRADES = [
  { level: 1, name: 'Basic Setup', cost: 2000, desc: 'Laptop, headphones, free DAW', quality: 1, icon: Headphones },
  { level: 2, name: 'Upgraded Gear', cost: 5000, desc: 'Audio interface, condenser mic, monitors', quality: 2, icon: Mic2 },
  { level: 3, name: 'Pro Home Studio', cost: 15000, desc: 'Soundproofing, pro gear, premium plugins', quality: 3, icon: MonitorSpeaker },
  { level: 4, name: 'Elite Studio', cost: 40000, desc: 'Industry-grade everything', quality: 4, icon: Speaker },
];

const GENRE_VIBES = ['Trap', 'R&B', 'Pop', 'Rock', 'Jazz', 'Lo-fi', 'Drill', 'Afrobeats', 'Reggaeton', 'Country'];

const SONG_NAME_TEMPLATES: Record<string, string[]> = {
  Trap: ['No Cap', 'Drip Season', 'Ice Cold', 'Bag Talk', 'Codeine Dreams', 'Foreign Whips'],
  'R&B': ['Midnight Serenade', 'Silk Sheets', 'Golden Hour', 'Tender Touch', 'Moonlight'],
  Pop: ['Neon Lights', 'Electric', 'Euphoria', 'Starlight', 'Levitate', 'Afterglow'],
  Rock: ['Thunderstruck', 'Concrete Jungle', 'Wildfire', 'Rebel Heart', 'Black Horizon'],
  Jazz: ['Blue Notes', 'Velvet Evening', 'Smoky Room', 'Satin Dreams', 'Late Night Set'],
  'Lo-fi': ['Rainy Afternoons', 'Coffee & Chill', 'Sunset Drive', 'Warm Static', 'Lazy Sunday'],
  Drill: ['Opp Block', 'Slide', 'No Lacking', 'Active', 'Wicked', 'Broadday'],
  Afrobeats: ['Lagos Nights', 'Soca Wine', 'Jollof', 'Dancefloor', 'Amapiano Dreams'],
  Reggaeton: ['Perreo', 'Fuego', 'La Calle', 'Baila Conmigo', 'Caliente'],
  Country: ['Dusty Roads', 'Pickup Truck', 'Honky Tonk', 'Southern Nights', 'Whiskey Sunset'],
  default: ['Midnight Dreams', 'On My Way', 'Feel the Vibe', 'City Lights', 'Ocean Eyes', 'Gravity', 'Runaway', 'Paradise', 'Faded', 'Unstoppable', 'Eclipse'],
};
...
export default function StudioApp({ profile }: Props) {
  ...
  const [selectedGenre, setSelectedGenre] = useState(profile.genre || 'Pop');
  ...
  const refreshSongs = async () => {
    const { data: vault } = await supabase.from('songs').select('*').eq('artist_id', profile.id).eq('streams', 0).order('created_at', { ascending: false });
    if (vault) setVaultSongs(vault);
    const { data: released } = await supabase.from('songs').select('*').eq('artist_id', profile.id).gt('streams', 0).order('streams', { ascending: false });
    if (released) setReleasedSongs(released);
  };
  ...
  const generateSongName = () => {
    const templates = SONG_NAME_TEMPLATES[selectedGenre] || SONG_NAME_TEMPLATES.default;
    setSongTitle(templates[Math.floor(Math.random() * templates.length)]);
  };
  ...
  const handleRecord = async () => {
    ...
    if (coverFile && user) coverUrl = await uploadArtistImage(coverFile, user.id);

    const { error: songError } = await supabase.from('songs').insert({
      artist_id: profile.id, title: songTitle.trim(), cover_url: coverUrl, streams: 0,
    });
    if (songError) { toast.error(songError.message); setRecording(false); return; }
    if (totalCost > 0) {
      await supabase.from('profiles').update({ current_money: profile.current_money - totalCost }).eq('id', profile.id);
    }
  ...
  const handleRelease = async (songId: string) => {
    ...
    if (profile.current_money < promoCost) { toast.error("You can't afford this promotion!"); setReleasing(false); return; }
    ...
    if (promo && promo.id !== 'none') {
      await supabase.from('promotions').insert({
        song_id: songId, artist_id: profile.id, promotion_type: promo.id, cost: promoCost, boost_multiplier: promoMultiplier,
      });
    }
  ...
  const upgradeHomeStudio = async (level: number, cost: number) => {
    if (!user) return;
    if (profile.current_money < cost) { toast.error("You can't afford this upgrade!"); return; }
    await supabase.from('profiles').update({
      has_home_studio: true, home_studio_level: level, current_money: profile.current_money - cost,
    }).eq('id', profile.id);
    toast.success(`Home studio upgraded to Level ${level}!`);
  };

  const homeStudioLevel = profile.home_studio_level || 0;
  const hasHomeStudio = profile.has_home_studio || false;

  const getStudioIcon = (emoji: string) => STUDIO_ICONS[emoji] || Music;
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-[#fff]">
      ...
      {section === 'main' && (
        <div className="px-4 space-y-4 pb-20">
          <div className="grid grid-cols-2 gap-3">
            ...
            <button onClick={() => setSection('analytics')} className="bg-gradient-to-br from-[#ef4444] to-[#dc2626] rounded-xl p-5 text-left">
          ...
          <div>
            ...
            <div className="space-y-3">
              {studios.map((studio) => {
                const StudioIcon = getStudioIcon(studio.image_emoji);
                return (
                  <button key={studio.id} onClick={() => { setSelectedStudio(studio); setSection('record'); }}
                    ...
                    <div className="w-14 h-14 rounded-xl bg-[#282828] flex items-center justify-center">
                      <StudioIcon className="w-7 h-7 text-[#1db954]" />
                    </div>
      ...
      {section === 'record' && (
        <div className="px-4 space-y-6 pb-20">
          <button onClick={() => setSection('main')} className="text-sm text-[#888] hover:text-white flex items-center gap-1">
            <Lock className="w-3 h-3" /> Back
          </button>

          {/* Genre selection */}
          <div>
            <h3 className="text-sm font-bold text-[#888] mb-2">Genre / Vibe</h3>
            <div className="flex gap-2 overflow-x-auto pb-2">
              {GENRE_VIBES.map((g) => (
                <button key={g} onClick={() => setSelectedGenre(g)}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${
                    selectedGenre === g ? 'bg-[#1db954] text-black' : 'bg-[#1a1a1a] text-white'
                  }`}>{g}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {studios.map((s) => {
                const SIcon = getStudioIcon(s.image_emoji);
                return (
                  <button key={s.id} onClick={() => setSelectedStudio(s)}
              ...
              {hasHomeStudio && (
                <button onClick={() => setSelectedStudio({ id: 'home', name: 'Home Studio', description: '', quality_level: homeStudioLevel, cost_per_session: 0, image_emoji: '🏠' })}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs whitespace-nowrap ${
                    selectedStudio?.id === 'home' ? 'bg-[#1db954] text-black font-bold' : 'bg-[#1a1a1a] text-white'
                  }`}>
                  <Home className="w-3.5 h-3.5" /> Home Studio (Free)
                </button>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={generateSongName} className="bg-[#282828] rounded-lg px-3 text-xs hover:bg-[#333] flex items-center gap-1">
                <Zap className="w-3.5 h-3.5 text-[#f59e0b]" /> Random
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <button onClick={() => fileRef.current?.click()}
              ...
              {coverPreview ? <img src={coverPreview} alt="" className="w-full h-full object-cover" /> : <Camera className="w-8 h-8 text-[#555]" />}
            </button>
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleCoverChange} />
          </div>

          <button onClick={handleRecord} disabled={recording || !songTitle.trim() || !selectedStudio}
            ...
            {recording ? (
              ...
              <><Mic2 className="w-5 h-5" /> Record Song {selectedStudio && selectedStudio.cost_per_session > 0 && `(${formatMoney(selectedStudio.cost_per_session)})`}</>
            )}
          </button>
        ...
        <div className="px-4 space-y-4 pb-20">
          ...
          {vaultSongs.length > 0 ? (
                ...
                <div key={song.id} className="bg-[#1a1a1a] rounded-xl p-4">
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    {PROMOTION_OPTIONS.map((promo) => {
                      ...
                      return (
                        <button key={promo.id} onClick={() => setSelectedPromo(promo.id)}
                          ...
                          <PromoIcon className="w-4 h-4 text-[#1db954]" />
                          <div className="flex-1">
                  ...
                  <button onClick={() => handleRelease(song.id)} disabled={releasing}
                    ...
                    {releasing ? 'Releasing...' : <><TrendingUp className="w-4 h-4" /> Release Song</>}
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-12">
              ...
              <button onClick={() => setSection('record')} className="mt-4 bg-[#1db954] text-black px-6 py-2 rounded-full text-sm font-bold">Record a Song</button>
            </div>
          )}
        </div>
      )}
      ...
      {section === 'analytics' && (
        <div className="px-4 space-y-4 pb-20">
          <button onClick={() => setSection('main')} className="text-sm text-[#888] hover:text-white">← Back</button>
          <h2 className="text-xl font-bold">Streaming Analytics</h2>

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#1a1a1a] rounded-xl p-4">
              <TrendingUp className="w-5 h-5 text-[#1db954] mb-2" />
              <p className="text-lg font-bold">{formatNumber(profile.total_streams)}</p>
              <p className="text-[10px] text-[#888]">Total Streams</p>
            </div>
            <div className="bg-[#1a1a1a] rounded-xl p-4">
              <Headphones className="w-5 h-5 text-[#8b5cf6] mb-2" />
              <p className="text-lg font-bold">{formatNumber(profile.monthly_listeners)}</p>
              <p className="text-[10px] text-[#888]">Monthly Listeners</p>
            </div>
            <div className="bg-[#1a1a1a] rounded-xl p-4">
              <DollarSign className="w-5 h-5 text-[#f59e0b] mb-2" />
              <p className="text-lg font-bold">{formatMoney(profile.current_money)}</p>
              <p className="text-[10px] text-[#888]">Balance</p>
            </div>
            <div className="bg-[#1a1a1a] rounded-xl p-4">
              <Disc3 className="w-5 h-5 text-[#ef4444] mb-2" />
              <p className="text-lg font-bold">{releasedSongs.length + vaultSongs.length}</p>
              <p className="text-[10px] text-[#888]">Total Songs</p>
            </div>
          </div>

          <div>
            <h3 className="font-bold mb-3">Top Songs</h3>
            <div className="space-y-2">
              {releasedSongs.slice(0, 10).map((song, i) => (
                <div key={song.id} className="flex items-center gap-3 bg-[#1a1a1a] rounded-xl p-3">
                  <span className="text-xs font-bold text-[#888] w-5 text-center">{i + 1}</span>
                  ...
                  <div className="w-20 h-2 bg-[#282828] rounded-full overflow-hidden">
                    <div className="h-full bg-[#1db954] rounded-full" style={{ width: `${Math.min(100, (song.streams / (releasedSongs[0]?.streams || 1)) * 100)}%` }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              return (
                <div key={upgrade.level} className={`bg-[#1a1a1a] rounded-xl p-4 ${isUnlocked ? 'ring-1 ring-[#1db954]' : ''}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <UpgradeIcon className="w-5 h-5 text-[#f59e0b]" />
                  ...
                  <div className="flex mb-2">
                    {Array.from({ length: upgrade.quality }).map((_, i) => (
                      <Star key={i} className="w-3 h-3 text-[#f59e0b] fill-[#f59e0b]" />
                    ))}
                  </div>
                  {isUnlocked ? (
                    <div className="text-xs text-[#1db954] font-medium flex items-center gap-1"><Check className="w-3 h-3" /> Unlocked</div>
                  ) : isNext ? (
                    <button onClick={() => upgradeHomeStudio(upgrade.level, upgrade.cost)} disabled={!canAfford}
                      className={`w-full py-2 rounded-lg text-sm font-bold ${canAfford ? 'bg-[#f59e0b] text-black' : 'bg-[#333] text-[#666]'}`}>
